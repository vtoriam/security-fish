...
def register():
    if request.method == "POST":
        ...
        if PendingUser.query.filter_by(username=username).first():
            flash("A request for that username is already pending approval.", "error")
            return render_template("register.html")
        ...
        # Admins bypass approval and are created immediately
        if role == "admin":
            user = User(
                username=username, display_name=display_name, role=role,
                avatar=initials or "?", color=color, text_color=text_color,
                mfa_secret=pyotp.random_base32(),
            )
            user.set_password(password)
            db.session.add(user)
            db.session.commit()
            log_event(username, "Registered as admin (bypass approval)", True)
            flash("Admin account created! Please log in.", "success")
            return redirect(url_for("login"))

        # All other roles go into the pending queue
        pending = PendingUser(
            username=username, display_name=display_name, role=role,
            avatar=initials or "?", color=color, text_color=text_color,
            mfa_secret=pyotp.random_base32(),
        ...
        db.session.commit()
        log_event(username, "Registration request submitted — awaiting approval", True)
        flash("Request submitted! An admin will review your account shortly. Come back and log in once approved.", "success")
        return redirect(url_for("login"))
...
# ---------------------------------------------------------------------------
# Delete account (own account only, requires password confirmation)
# ---------------------------------------------------------------------------
...
def delete_account():
    if request.method == "POST":
        ...
        if user.role == "admin" and User.query.filter_by(role="admin").count() <= 1:
            flash("Cannot delete the only admin account. Promote another user to admin first.", "error")
            return render_template("delete_account.html")

        username = user.username
        db.session.delete(user)   # cascade deletes vault entries too
        db.session.commit()
        log_event(username, "Account deleted", True)
        session.clear()
...
def admin_reset_password(user_id):
    ...
    # Admins cannot reset their own password via this route (use change-password)
    ...
    if request.method == "POST":
        ...
        # Force MFA re-setup so the user scans a fresh QR on next login
        target.mfa_secret   = pyotp.random_base32()
        target.mfa_verified = False
        db.session.commit()
        log_event(session["username"],
                  f"Reset password for {target.username}", True)
        flash(f"Password reset for {target.display_name}. They will need to re-scan their MFA QR code on next login.", "success")
        return redirect(url_for("dashboard"))
...
def approve_user(pending_id):
    ...
    if User.query.filter_by(username=pending.username).first():
        ...
        db.session.commit()
        flash(f"Username '{pending.username}' already taken — request removed.", "warning")
        return redirect(url_for("dashboard"))
    ...
    db.session.commit()
    log_event(session["username"],
              f"Approved registration: {user.username} ({user.role})", True)
    flash(f"{user.display_name}'s account approved — they can now log in.", "success")
    return redirect(url_for("dashboard"))
...
def reject_user(pending_id):
    ...
    db.session.delete(pending)
    db.session.commit()
    log_event(session["username"], f"Rejected registration: {pending.username}", True)
    flash(f"{name}'s registration request has been rejected and removed.", "success")
    return redirect(url_for("dashboard"))
...
def dashboard():
    ...
    pending = PendingUser.query.order_by(PendingUser.requested_at.asc()).all() if can("manage_members") else []
